...
class Infiller: 
    ...
    def generate(self, input_ids, num_return_sequences, max_to_generate: int=52, temperature : float=0.5):
        ...
        with torch.no_grad():
            output = self.model.generate(input_ids=input_ids, do_sample=True, top_p=0.95, temperature=0.5, max_length=max_length, return_dict_in_generate=True, output_scores=True, num_return_sequences=num_return_sequences)
    ...
    def entropy(self, input_ids, start_loc, target_len):
        ...
        with torch.no_grad():
            ...
            #print("LOSS: ", loss)
            #make a list of the loss for each token
